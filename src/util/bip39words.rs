...
const BIP39_WORDS_COUNT: u64 = BIP39_WORDS.len() as u64;

pub fn to_bip39_words(number: u64) -> String {
    let mut result: Vec<&str> = Vec::new();

    if number == 0 {
        return BIP39_WORDS[0].to_string();
    }

    // Convert to little-endian representation first
    let mut value = number;
    while value != 0 {
        let digit = (value % BIP39_WORDS_COUNT) as usize;
        value /= BIP39_WORDS_COUNT;
        result.push(BIP39_WORDS[digit]);
    }

    // Convert to big-endian for final output
    result.reverse();
    result.join("-")
}

pub fn to_u64(bip39_words: &str) -> Result<u64, &str> {
    let mut result: u64 = 0;

    for word in bip39_words.split('-') {
        let word_index = match BIP39_WORDS.iter().position(|&r| r == word) {
            Some(idx) => idx,
            None => return Err("Failed to convert BIP39 word to u64!"),
        };

        // Multiply by word count before adding next value to maintain correct order
        result = result * BIP39_WORDS_COUNT + (word_index as u64);
    }
...
mod tests {
    ...
    #[test]
    fn test_to_bip39_words() {
        assert_eq!(to_bip39_words(0), "abandon");
        assert_eq!(to_bip39_words(1), "ability");
        assert_eq!(to_bip39_words(2048), "ability-abandon");
        // Add more test cases as needed
    }

    #[test]
    fn test_bip39_to_u64() {
        assert_eq!(to_u64("abandon"), Ok(0));
        assert_eq!(to_u64("ability"), Ok(1));
        assert_eq!(to_u64("ability-abandon"), Ok(2048));
        assert_eq!(
            to_u64("not-a-word"),
            Err("Failed to convert BIP39 word to u64!")
        );
        // Add more test cases as needed
    }
    ...
    #[test]
    fn test_roundtrip() {
        let numbers = vec![0, 1, 2048, 12345, 1000000];
        for num in numbers {
            let words = to_bip39_words(num);
            let result = to_u64(&words).unwrap();
            assert_eq!(result, num, "Failed roundtrip for number {}", num);
        }
    }
}
